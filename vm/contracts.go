...
// StatefulPrecompiledContract is an extended interface that allows access to StateDB.
type StatefulPrecompiledContract interface {
	RequiredGas(input []byte) uint64
	RunStateful(input []byte, statedb StateDB) ([]byte, error)
	Name() string
}
...
func activePrecompiledContracts() PrecompiledContracts {
	return PrecompiledContractsCancun
}

// ActivePrecompiledContracts returns a copy of precompiled contracts enabled with the current configuration.
func ActivePrecompiledContracts() PrecompiledContracts {
	return maps.Clone(activePrecompiledContracts())
}
...
func (b *kzgPointEvaluation) RequiredGas(input []byte) uint64 {
	return BlobTxPointEvaluationPrecompileGas
}
