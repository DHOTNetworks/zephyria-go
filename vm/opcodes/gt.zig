...
fn execute(evm: *EVM) !void {
    // EVM: GT pops a (TOS), then b, pushes a > b
    if (evm.stack.pop()) |a| {
        if (evm.stack.pop()) |b| {
            try evm.stack.push(evm.allocator, if (a.gt(b)) @import("../main.zig").BigInt.init(1) else @import("../main.zig").BigInt.init(0));
...
pub fn jit_compile(jit: anytype, pc: *usize, stack_top: *u64, bytecode: []const u8) !void {
    ...
    // GT: a > b where a=TOS (s1), b=TOS-1 (s2) -> emit_native_gt(dst, a, b)
    try jit.emit_native_gt(s2_reg, s1_reg, s2_reg);
    jit.pop_virtual(1);
    stack_top.* -= 1;
}
