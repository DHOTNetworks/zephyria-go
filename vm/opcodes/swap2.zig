...
fn execute(evm: *EVM) !void {
    // Swap the top item with the third item (indices 0 and 2 from top)
    if (evm.stack.items.items.len >= 3) {
        const len = evm.stack.items.items.len;
        const temp = evm.stack.items.items[len - 1]; // Top
        evm.stack.items.items[len - 1] = evm.stack.items.items[len - 3]; // Third from top
        evm.stack.items.items[len - 3] = temp;
