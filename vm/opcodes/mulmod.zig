...
pub fn jit_compile(jit: anytype, pc: *usize, stack_top: *u64, bytecode: []const u8) !void {
    ...
    if (v_a == .constant and v_b == .constant and v_n == .constant) {
        ...
        var result: u256 = 0;
        if (n != 0) {
            // Use wrapping mul then mod
            const product = v_a.constant *% v_b.constant;
            result = product % n;
        }
        jit.pop_virtual(3);
    ...
    try jit.emit_native_mulmod(n_slot.register, a_slot.register, b_slot.register, n_slot.register);
    jit.pop_virtual(2);
    stack_top.* -= 2;
}
