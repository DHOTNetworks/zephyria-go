...
// Zephyria Token Proxy Bytecode (EIP-1167 to 0x...5000)
// 363d3d373d3d3d363d73 + 0000000000000000000000000000000000005000 + 5af43d82803e903d91602b57fd5bf3
var TokenProxyCode = common.FromHex("363d3d373d3d3d363d7300000000000000000000000000000000000050005af43d82803e903d91602b57fd5bf3")

func isTokenProgramOrProxy(evm *EVM, addr common.Address) bool {
	// 1. Logic Contract itself
	if addr == state.TokenProgramID {
		return true
	}
	// 2. Direct match (Owner set via ProgramAddress field)
	if evm.StateDB.GetProgramAddress(addr) == state.TokenProgramID {
		return true
	}
	// 3. Proxy Code match (Backup safety)
	code := evm.StateDB.GetCode(addr)
	if len(code) == len(TokenProxyCode) && bytes.Equal(code, TokenProxyCode) {
		return true
	}
	return false
}
...
func opSload(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	...
	// AQUARIUS: Intercept SLOAD for Token Program
	// Distinguish between Metadata (Slot 0, 1...) and Balances (Key = Address)
	addr := scope.Contract.Address()
	if isTokenProgramOrProxy(evm, addr) {
		// Heuristic: Metadata slots are small integers. Addresses are huge.
		// Threshold: 0x100000 (allows for array length slots and other metadata)
		// If key < Threshold, it's metadata.
		// If key >= Threshold, we assume it's an Address (Balance mapping).

		limit := new(big.Int).SetUint64(0x100000)
		if loc.ToBig().Cmp(limit) < 0 {
			// Metadata -> Read from Contract Storage directly
			val := evm.StateDB.GetState(addr, hash)
			loc.SetBytes(val.Bytes())
			return nil, nil
		}

		// Sharded Data (Balance)
		keyBytes := loc.Bytes32()
		owner := common.BytesToAddress(keyBytes[:])

		// Derive ATA (Data Shard)
		targetATA := state.DeriveATA(owner, addr)

		val := evm.StateDB.GetState(targetATA, hash)
		fmt.Printf(" [⚖] AQUARIUS: Intercepted Balance READ [Owner: %s | Mint: %s | Result: %s]\n",
			owner.Hex(), addr.Hex(), val.Big().String())
		loc.SetBytes(val.Bytes())
		return nil, nil
	}
...
func opSstore(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	...
	// AQUARIUS: Intercept SSTORE for Token Program
	addr := scope.Contract.Address()
	if isTokenProgramOrProxy(evm, addr) {
		limit := new(big.Int).SetUint64(0x100000)
		...
		if loc.ToBig().Cmp(limit) < 0 {
			hash := common.Hash(loc.Bytes32())
			evm.StateDB.SetState(addr, hash, common.Hash(val.Bytes32()))
			return nil, nil
		}
		...
		// SetState implicitly creates/updates storage.
		// Zephyria Statedb should handle it.
		// But ideally we ensure the ATA account object exists via EnsureDataAccount logic,
		// For robustness, simply writing to State is enough for PoC.
		fmt.Printf(" [⚖] AQUARIUS: Intercepted Balance WRITE [Owner: %s | Mint: %s | Value: %s | ATA: %s]\n",
			owner.Hex(), addr.Hex(), val.ToBig().String(), targetATA.Hex())

		evm.StateDB.SetState(targetATA, hash, common.Hash(val.Bytes32()))
		return nil, nil
	}

	// Safe logging: avoid slicing if hex string is too short
	locHex := loc.Hex()
	valHex := val.Hex()
	if len(locHex) > 10 {
		locHex = locHex[:10]
	...
	fmt.Printf(" [💾] VM: SSTORE [Addr: %s | Loc: %s | Val: %s]\n",
		scope.Contract.Address().Hex(), locHex, valHex)
...
// opEofCreate implements the EOFCREATE instruction (EIP-7620)
func opEofCreate(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	if evm.readOnly {
		return nil, ErrWriteProtection
	}

	// Stack: initcontainer_index, endowment, salt, data_offset, data_size
	initContainerIdx := scope.Stack.pop()
	endowment := scope.Stack.pop()
	salt := scope.Stack.pop()
	dataOffset := scope.Stack.pop()
	dataSize := scope.Stack.pop()

	// 1. Charge Gas (simplified)
	gas := scope.Contract.Gas
	gas -= gas / 64
	scope.Contract.UseGas(gas, evm.Config.Tracer, tracing.GasChangeCallContractCreation)

	// 2. Get Initcode from Subcontainer (Stub)
	// In real impl, use scope.Contract.GetSubContainer(initContainerIdx.Uint64())
	// Assuming empty for PoC if not implemented.
	_ = initContainerIdx
	var initCode []byte

	// 3. Calldata for Initcode
	input := getData(scope.Memory.Data(), dataOffset.Uint64(), dataSize.Uint64())

	// 4. EofCreate behavior
	res, addr, returnGas, suberr := evm.EofCreate(scope.Contract.Address(), initCode, input, gas, &endowment, &salt)

	// EIP-7620: Returns address on stack
	retStack := new(uint256.Int)
	if suberr != nil {
		retStack.Clear()
	} else {
		retStack.SetBytes(addr.Bytes())
	}
	scope.Stack.push(retStack)
	scope.Contract.RefundGas(returnGas, evm.Config.Tracer, tracing.GasChangeCallLeftOverRefunded)
...
// opReturnContract implements RETURNCONTRACT (EIP-7620)
func opReturnContract(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	deployIdx := scope.Stack.pop()
	auxOffset := scope.Stack.pop()
	auxSize := scope.Stack.pop()

	if !scope.Contract.IsDeployment {
		return nil, ErrInvalidJump
	}

	// Stub: Return empty code for now.
	// Real impl: construct code from subcontainer[deployIdx] + auxData
	_ = deployIdx
	_ = auxOffset
	_ = auxSize

	// This acts as a RETURN but for deployment
	return nil, nil
}

// opTxCreate implements TXCREATE (EIP-7873)
func opTxCreate(pc *uint64, evm *EVM, scope *ScopeContext) ([]byte, error) {
	if evm.readOnly {
		return nil, ErrWriteProtection
	}
	// Stack: initcontainer_index, endowment, salt, data_offset, data_size
	...
	scope.Stack.push(new(uint256.Int).Clear()) // Fail by default in stub
	return nil, nil
}
