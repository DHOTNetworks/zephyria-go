...
fn execute(evm: *EVM) !void {
    // Duplicate the 16th stack item (index 16 from top)
    if (evm.stack.items.items.len >= 16) {
        const len = evm.stack.items.items.len;
        const value = evm.stack.items.items[len - 16]; // 16th from top
