...
fn execute(evm: *EVM) !void {
    // Duplicate the 11th stack item (index 11 from top)
    if (evm.stack.items.items.len >= 11) {
        const len = evm.stack.items.items.len;
        const value = evm.stack.items.items[len - 11]; // 11th from top
