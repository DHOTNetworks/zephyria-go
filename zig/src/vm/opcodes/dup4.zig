...
fn execute(evm: *EVM) !void {
    // Duplicate the 4th stack item (index 4 from top)
    if (evm.stack.items.items.len >= 4) {
        const len = evm.stack.items.items.len;
        const value = evm.stack.items.items[len - 4]; // 4th from top
