...
use interpreter::SharedMemory;
use primitives::{Env, SpecId};
...
fn run_bench(c: &mut Criterion, bench: &Bench) {
    ...
    let bytecode = interpreter::analysis::to_analysed(primitives::Bytecode::new_raw(
        primitives::Bytes::copy_from_slice(bytecode),
    ));
    let contract = interpreter::Contract::new_env(&env, bytecode, None);
    let mut host = interpreter::DummyHost::new(env);

    let bytecode = contract.bytecode.original_byte_slice();

    let table = &interpreter::opcode::make_instruction_table::<
    ...
    let mut call_jit = |f: EvmCompilerFn| {
        ...
        let mut interpreter =
            interpreter::Interpreter::new(contract.clone(), gas_limit, false);
        host.clear();
    ...
    g.bench_function("revm-interpreter", |b| {
        b.iter(|| {
            let mut int = interpreter::Interpreter::new(contract.clone(), gas_limit, false);
