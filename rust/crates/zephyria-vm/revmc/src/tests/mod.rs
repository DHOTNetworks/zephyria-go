...
use primitives::{hex, keccak256, Address, Bytes, LogData, B256, KECCAK_EMPTY};
    ...
    host {
        ...
        eofcreate(@raw {
            ...
            expected_next_action: InterpreterAction::EOFCreate {
                inputs: Box::new(interpreter::EOFCreateInputs {
                    caller: DEF_ADDR,
                    value: 0x42_U256,
                    gas_limit: 66899,
                    kind: interpreter::EOFCreateKind::Opcode {
...
fn eof_sections(code: &[&[u8]]) -> Bytes {
    ...
    match interpreter::analysis::validate_eof_inner(&eof, None) {
        Ok(()) => {}
