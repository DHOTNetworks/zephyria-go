...
use interpreter::{opcode as op, Contract, DummyHost, Host, SelfDestructResult};
use primitives::{
...
impl<'a> arbitrary::Arbitrary<'a> for TestCase<'a> {
    fn arbitrary(u: &mut arbitrary::Unstructured<'a>) -> arbitrary::Result<Self> {
        ...
        if is_eof_enabled && !bytecode.starts_with(&EOF_MAGIC_BYTES) && u.arbitrary()? {
            ...
            if interpreter::analysis::validate_eof(&code).is_ok() {
...
pub fn with_evm_context<F: FnOnce(&mut EvmContext<'_>, &mut EvmStack, &mut usize) -> R, R>(
    ...
    let contract = Contract {
        ...
        bytecode: interpreter::analysis::to_analysed(primitives::Bytecode::new_raw(
    ...
    let mut interpreter = interpreter::Interpreter::new(contract, DEF_GAS_LIMIT, false);
...
impl fmt::Debug for MemDisplay<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let chunks = self.0.chunks(32).map(primitives::hex::encode_prefixed);
