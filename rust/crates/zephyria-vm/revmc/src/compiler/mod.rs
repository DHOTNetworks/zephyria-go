...
use interpreter::{Contract, Gas};
use primitives::{Bytes, Env, Eof, SpecId, EOF_MAGIC_BYTES};
use revmc_backend::{
    eyre::{ensure, eyre},
    Attribute, FunctionAttributeLocation, Linkage, OptimizationLevel,
};
use revmc_builtins::Builtins;
use context::RawEvmCompilerFn;
use std::{
    borrow::Cow,
    fs,
    io::{self, Write},
...
impl<B: Backend> EvmCompiler<B> {
    ...
    fn do_validate_eof(&self, eof: &Eof) -> Result<()> {
        ...
        interpreter::analysis::validate_eof_inner(eof, None).map_err(|e| match e {
            interpreter::analysis::EofError::Decode(e) => e.into(),
            interpreter::analysis::EofError::Validation(e) => {
