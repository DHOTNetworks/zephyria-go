...
use primitives::{BlockEnv, CfgEnv, Env, Eof, SpecId, TxEnv, U256};
use revmc_backend::{
...
impl<'a, B: Backend> FunctionCx<'a, B> {
    ...
    fn translate_inst(&mut self, inst: Inst) -> Result<()> {
        ...
        if !is_eof && self.config.stack_bound_checks {
            ...
            if diff > context::EvmStack::CAPACITY as i64 {
...
mod pf {
    ...
    const _: [(); mem::size_of::<primitives::Bytes>()] = [(); mem::size_of::<Bytes>()];
    struct Vtable {
        /// fn(data, ptr, len)
    ...
    const _: [(); mem::size_of::<interpreter::Gas>()] = [(); mem::size_of::<Gas>()];
    ...
    const _: [(); mem::size_of::<interpreter::SharedMemory>()] =
        [(); mem::size_of::<SharedMemory>()];

    #[test]
    fn shared_memory_layout() {
        let mem = interpreter::SharedMemory::default();
