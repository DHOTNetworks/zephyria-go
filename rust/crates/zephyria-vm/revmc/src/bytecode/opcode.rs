...
use interpreter::{opcode as op, OPCODE_INFO_JUMPTABLE};
use primitives::{Bytes, Eof, SpecId, EOF_MAGIC_BYTES};
use std::{fmt, slice};
...
impl fmt::Display for Opcode<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self.immediate {
            Some(imm) => write!(f, " {}", primitives::hex::encode_prefixed(imm)),
            None => Ok(()),
        }
    }
}
