...
/// Implements the DATACREATE instruction.
///
/// Creates a new Data Account linked to a Program.
pub fn datacreate<WIRE: InterpreterTypes, H: Host + ?Sized>(
    context: InstructionContext<'_, H, WIRE>,
) {
    popn!(
        [value, code_offset, len, salt, program_address],
        context.interpreter
    );
    let len = as_usize_or_fail!(context.interpreter, len);
    let program_address = Address::from_word(B256::from(program_address));
    ...
    // Gas Cost (Same as Create2 for now)
    gas!(
        context.interpreter,
        context.host.gas_params().create2_cost(len)
    );

    let scheme = CreateScheme::DataCreate {
        salt: salt.into(),
        program_address,
    };
