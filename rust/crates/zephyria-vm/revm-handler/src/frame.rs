...
impl EthFrame<EthInterpreter> {
    ...
    >(
        ...
        let created_address = match inputs.scheme() {
            ...
            CreateScheme::DataCreate {
                salt,
                program_address,
            } => {
                let program_hash = keccak256(program_address.as_slice());
                inputs.caller().create2(salt.to_be_bytes(), program_hash)
            }
        };
    ...
    > {
        ...
        match frame_input {
            ...
            FrameInput::EOFCreate(_inputs) => {
                // TODO: Implement EOF create when EOF support is complete
                unimplemented!("EOF create not yet implemented")
            }
