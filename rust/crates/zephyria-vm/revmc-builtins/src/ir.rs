...
impl<B: Backend> Builtins<B> {
    ...
    fn init(builtin: Builtin, bcx: &mut B::Builder<'_>) -> B::Function {
        ...
        bcx.get_function(name)
            .inspect(|r| trace!(name, ?r, "pre-existing"))
            .unwrap_or_else(|| {
...
builtins! {
    ...
    @param_attrs |op| {
        ...
        let op = op.op();
        let (inputs, outputs) = if let Some(info) = interpreter::opcode::OPCODE_INFO_JUMPTABLE[op as usize] {
            (info.inputs(), info.outputs())
        } else {
            (0, 0)
        };

        let ecx = size_and_align::<context::EvmContext<'static>>();

        let sp_dyn = size_and_align2(None, core::mem::align_of::<context::EvmWord>());
        ...
        let size_of_word = core::mem::size_of::<context::EvmWord>();
